...
| MODEL_NAME | | str | Model name when using the API to access LLMs. If not filled in, the default is gpt-4o. Qwen/Qwen3-8B is recommended when using [SiliconFlow](https://cloud.siliconflow.cn/i/b3XhBRAm). | Qwen/Qwen3-8B |
...
```
> [!IMPORTANT]
> The workflow will download and run an LLM (Qwen3-4B, the file size of which is about 3G). Make sure your network and hardware can handle it.
...
The TLDR of each paper is generated by a lightweight LLM (Qwen3-4b-q4_k_m), given its title, abstract, introduction, and conclusion (if any). The introduction and conclusion are extracted from the source latex file of the paper.
